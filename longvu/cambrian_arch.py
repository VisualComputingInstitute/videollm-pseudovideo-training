...
class CambrianMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        image_sizes=None,
        also_return_aligned_embeds=False
    ):
        """
            input_ids: the text token IDs after tokenization
            position_ids: positional embeddings to be used? For the whole
            sequence, or just for the text?
            attention_mask: indicates which (image?) tokens are padding
            past_key_values: used to speed up autoregressive computation
            labels: the text expected as prediction (is None during
            inference)
            images: all video frames to be processed by LongVU
            image_aux_attention_masks_list: tokens that are to be treated
            as padding for the vision encoders (can differ if e.g.
            Dino and SigLIP use different resolutions)
            image_sizes: the sizes of input images

        """
        # vision_tower = self.get_vision_tower()
        vision_tower_aux_list = self.get_model().get_vision_tower_aux_list()
        #if the model is not equipped with vision encoders, or no images were
        #fed (or if only one text token is given???)
        if(vision_tower_aux_list is None or images is None 
           or input_ids.shape[1] == 1):
            #don't do anything image-related
            return (
                input_ids,
                position_ids,
                attention_mask,
        ...
        split_sizes = None
        #check if images are videos 
        #(5 = {batch, sequence, height, width, channels})
        if type(image_aux_list[0]) is list or image_aux_list[0].ndim == 5:
            #get the sequence lengths per batch element?
            split_sizes_ori = [
                1 if image.ndim == 3 else image.shape[0] 
                for image in image_aux_list[0]]
            new_image_aux_list = []
            for image_aux in image_aux_list: #iterate over encoders
                if type(image_aux) is list:
                    #add unitary dimension to 1-image "videos"
                    image_aux = [
                        x.unsqueeze(0) if x.ndim == 3 else x for x in image_aux
                    ]
                #make a big batch for the encoder by collapsing the time dim.
                concat_image_aux = torch.cat([
                    image for image in image_aux], dim=0)
                new_image_aux_list.append(concat_image_aux)
            #first encode with dino, and then determine which frames to keep
            ...
            (
                ...
                image_sizes,
                threshold=getattr(
                    self.get_model().config, "dino_threshold", 0.83),
            )
            #for the frames that survived, encode with siglip
            ...
            #everything is simpler in the single-image case (no frame drops)
            image_aux_features_list = self.encode_images(image_aux_list)
            bs = image_aux_list[0].shape[0]
            dtype = image_aux_list[0].dtype
        #at this point bs is the batch size
        #image_token_len is the max. num. of tokens to be used per frame
        #query_num_list is sth. to do with Cambrian's SVA
        image_token_len = self.get_model().config.image_token_len
        query_num_list = self.get_model().config.query_num_list
        #interpret image_token_len as the num. of patches in a square
        ...
        if self.get_model().config.mm_projector_type == "sva":
            ...
            #is this to make sure that the feat. dim. is the same everywhere?
            for aux_i in range(len(vision_tower_aux_list)):
            ...
            for query_group_i, query_num in enumerate(query_num_list):
                ...
                if IS_XLA_AVAILABLE:
                    ...
                    (
                        ...
                        vision_tower_aux_feature_list, query_side_len, 
                        image_sizes)
                #attend?
                ...
                #figure out how many frames are allowed to maintain
                #original resolution
                if split_sizes is not None:
                    try:
                        if "llama" in self.get_model().config.model_type:
                            text_len = torch.where(
                                input_ids[0] == 128002)[-1][0]
                        else:
                            text_len = torch.where(
                                input_ids[0] == 151643)[-1][0]
                    except:
                        text_len = len(input_ids[0])
                    max_visual_len = (
                        self.get_model().config.tokenizer_model_max_length
                        - text_len
                        - getattr(self.get_model().config, 
                                  "inference_max_length", 16)
                    )
                    max_num_frames = max(
                        1,
                        math.floor(
                            max_visual_len // (final_height * final_width)),)
                    max_num_frames_low = max(
                        1,
                        math.floor(
                            max_visual_len
                            // (self.get_model().config.lowres_token ** 2)),)
                    #check whether all frames can be maintained or not
                ...
                if (getattr(self.config, "highres", False)) and input_mix_res:
                    #this is where the low- resolution features are computed
                    _query_features_i = (
                    ...
                    _query_features_i = (
                        _query_features_i.permute(0, 2, 3, 1)
                        .contiguous().flatten(1, 2))
            ...
            if IS_XLA_AVAILABLE:
                ...
                global_context_feature_final = global_context_feature.expand(
                    -1, final_height * final_width, 1, -1).flatten(0, 1)
        else:
            final_image_features_list = image_aux_features_list
        #align the features with the LLM's input space
        image_features = torch.cat(final_image_features_list, -1)
        image_features = self.get_model().mm_projector(
            image_features).to(dtype)
        aligned_img_feats = image_features.clone().detach()
        #same for the low-res features
        if (getattr(self.config, "highres", False)) and input_mix_res:
            image_features_down = torch.cat(final_image_features_down_list, -1)
            image_features_down = (
                self.get_model().mm_projector(image_features_down).to(dtype)
            )
        # what hasn't happened yet at this point:
        # 1) determining which frames to keep at high-res
        # 2) token dropping inside each sub-window of size K (see paper)
        if IS_XLA_AVAILABLE:
            image_features = image_features.view(
                image_features.shape[0], final_height, final_width, -1
            )
            if(getattr(self.config, "is_image_newline", True)):
            ...
            if self.get_model().config.mm_projector_type == "sva":
                (
                    ...
                    vision_tower_aux_feature_list, final_height, image_sizes, 
                    unpad=True)
            ...
            for batch_i in range(bs):
                ...
                if (getattr(self.config, "highres", False)) and input_mix_res:
                    ...
                    try:  # fix bug for some invalid image
                        ...
                        cur_image_feature_down = image_features_down[
                            batch_i].unsqueeze(0)
                        _cur_h, _cur_w = cur_image_feature_down.shape[1:3]
                        cur_image_feature_down = cur_image_feature_down.view(
                            1, _cur_h, _cur_w, -1)
                    if(getattr(self.config, "is_image_newline", True)):
                        cur_image_feature_down = torch.cat(
                            ...
                            dim=2,).flatten(1, 2)
                    #maybe add frame positional embeddings
                    if split_sizes is None and getattr(
                        self.config, "frame_pos", False):
                        frame_pos = (
                            self.get_model()
                    ...
                    image_features_downsample.append(
                        cur_image_feature_down.squeeze(0))
                if(getattr(self.config, "is_image_newline", True)):
                    cur_image_feature = torch.cat((
                        cur_image_feature,
                        self.model.image_newline.view(1, 1, 1, -1)
                        .expand(1, cur_h, 1, -1)
                        .to(cur_image_feature.device),), dim=2,)

                if split_sizes is None and getattr(self.config, "frame_pos", 
                                                   False):
                    frame_pos = (self.get_model()
                        .get_frame_pos(torch.arange(1))
                ...
                if self.get_model().config.mm_projector_type == "sva":
                    cur_global_context_feature = global_context_feature[
                        batch_i].expand(cur_h * cur_w, 1, -1)
                    global_context_feature_final.append(
                        cur_global_context_feature)
            if self.get_model().config.mm_projector_type == "sva":
        ...
        if position_ids is None:
            position_ids = torch.arange(
                0, input_ids.shape[1], dtype=torch.long, 
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(len(image_token_indices) - 1):
                ...
                cur_labels_noim.append(
                    cur_labels[image_token_indices[i] + 1 
            ...
            if (input_mix_res
                ...
                if max_visual_len > visual_len:
                    ...
                    highres_num = math.floor(
                        ...
                        / (
                            split_image_features_unpadded[
                                cur_image_idx].shape[0]
                            // frame_split_sizes[cur_image_idx]
                            - visual_emb.shape[0] // frame_split_sizes[
                                cur_image_idx]))
                    if highres_num >= 1:
                        mix_token = True
                        sim = torch.matmul(visual_emb, text_emb.transpose(
                            0, 1)).mean(dim=-1)
                        sim_frame = sim.reshape(
                            frame_split_sizes[cur_image_idx], -1).mean(dim=-1)
                        highres_num = min(highres_num, sim_frame.shape[0])
                        top_values, top_indices = torch.topk(
                            sim_frame, highres_num)
                        if len(top_indices) > 0:
                            sorted_indices = torch.sort(top_indices)[1]
                            top_indices = top_indices[sorted_indices]
                            visual_emb_frame = image_features[
                                cur_image_idx].reshape(
                                    frame_split_sizes[cur_image_idx], -1,
                                    image_features[cur_image_idx].shape[-1],)
                            visual_emb_frame_highres = (
                                split_image_features_unpadded[cur_image_idx]
                                .reshape(frame_split_sizes[cur_image_idx], -1,
                                split_image_features_unpadded[
                                    cur_image_idx].shape[-1],))
                            current_point = 0
            ...
            if (max_visual_len < visual_len
                ...
                for start_idx in range(0, len(visual_emb_frame), 8):
                    ...
                    new_visual_emb_frame = torch.cat([
                        chunk_feature[0], chunk_feature[1:].flatten(0, 1)[
                            sim.flatten(0, 1) < getattr(
                                self.get_model().config, 
                                "drop_threshold", 0.7)],], dim=0,)
                    new_visual_emb_frames.append(new_visual_emb_frame)

                reduced_visual_len = sum([
                    x.shape[0] for x in new_visual_emb_frames])
                ...
                if reduced_visual_len > max_visual_len:
                    ...
                    for chunk_i in range(len(new_visual_emb_frames)):
                        new_visual_emb_frames[chunk_i] = new_visual_emb_frames[
                            chunk_i][:-force_remove]
                    new_visual_emb_frames = torch.cat(new_visual_emb_frames, 
                                                      dim=0)
                else:
                    new_visual_emb_frames = torch.cat(new_visual_emb_frames, 
                                                      dim=0)

                image_features[cur_image_idx] = new_visual_emb_frames[
                    :max_visual_len]
        ...
        for i, (cur_new_embed, cur_new_labels) in enumerate(
            ...
            if getattr(self.config, "tokenizer_padding_side", 
                ...
                new_input_embeds_padded.append(
                    torch.cat((cur_new_embed, torch.zeros(
                        (max_len - cur_len, cur_new_embed.shape[1]),
        ...
        return ((
            ...
            final_size,
            global_context_feature_final,) +
            ((selected_frame_indices_all, aligned_img_feats)
            if also_return_aligned_embeds else ()))
