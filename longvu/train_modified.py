...
#I don't see how the next imports ever worked without this
sys.path.append(os.path.abspath("./"))

from longvu import conversation as conversation_lib
from longvu.custom_datasets.longvu_dataset_wrappers import (
    WrappedMotionDatasetLazy, WrappedSceneDescrDataset, 
    WrappedSplicedImgsDataset, WrappedSplicedImgsDatasetWithMasks)
from longvu.custom_datasets.dataset_instantiation import (
    motion_dataset_from_yaml, 
    scene_descr_dataset_from_yaml,
    spliced_coco_imgs_dataset_from_yaml)
from longvu.custom_datasets.multiq_linear_motion_dataset import \
    MultiQuestionLinMotDataset
from longvu.custom_datasets.img_splicing_dataset import (
    ImgSplicingDataset,
    ImgSplicingDatasetWithObjInfo)
from longvu.custom_datasets.motion_question_generators import (
    MovementDirectionQGen,
    StillObjectQGen,
    NotPresentObjectQGen,
    ObservationQGen,
    MovementInReverseQGen)
from longvu.custom_datasets.counting_question_generators import (
    HowManyObjsQGen,
...
from longvu.custom_datasets.spliced_imgs_q_gens import (
    ScenesDescrQGen, 
    DescribeNthSceneQGen, 
    CountScenesQGen,
    DescrAdjacentSceneQGen,
    FrLvlAnnotationQGen,
    BeforeOrAfterQGen,
    WhichHappensFirstLastQGen,
    WhichIsOrIsntInVideoQGen)
from longvu.custom_datasets.spliced_objs_q_gens import (
    HowManyInstOfClassQGen)
...
from longvu.mm_datautils import (
    ...
    videollava_safe_save_model_for_hf_trainer,
    smart_tokenizer_and_embedding_resize,
)
from longvu.mm_trainer import LLaVATrainer, VALID_CKPT_FILE
...
class DatasetType(str, Enum):
    ORIGINAL_LONGVU="original_longvu"
    LINEAR_MOTION="linear_motion"
    SCENE_DESCR="scene_descr"
    COCO_IMG_SPLICING="coco_img_splicing"

@dataclass
class DataArguments:
    dataset_type: DatasetType = field(default=DatasetType.ORIGINAL_LONGVU)
    dataset_kwargs_as_dict_str: str = field(
        default="\{\}")
    repeat_first_frame_only: bool = field(default=False)
    debug_print_question: bool = field(default=False)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        # pyre-fixme[4]: Attribute must be annotated.
        self.length = 12 #self._get_length()
    ...
    def _compute_lengths(self):
        ...
        for sample in self.list_data_dict[:12]:
            img_tokens = (
        ...
        if has_image:
            if "image" in dat:
                ...
                if video_file.endswith(".gif"):
                    ...
                    video_file = (
                        "/p/project1/efficientvid/workspaces/longvu_workspace"
                        "/training_data/motion_test/easiest_train_000347.mp4")                    
...
def make_supervised_data_module(
    dataset_type: DatasetType, dataset_kwargs_as_dict_str: str,
    tokenizer: transformers.PreTrainedTokenizer, data_args,  # pyre-fixme
    training_args) -> Dict:  # pyre-fixme
    """Make dataset and collator for supervised fine-tuning."""

    MultiQuestionLinMotDataset.register_question_generator(
        "movement_dir", MovementDirectionQGen)
    MultiQuestionLinMotDataset.register_question_generator(
        "still_object", StillObjectQGen)
    MultiQuestionLinMotDataset.register_question_generator(
        "not_present_object", NotPresentObjectQGen)
    MultiQuestionLinMotDataset.register_question_generator(
        "movement_description", ObservationQGen)
    MultiQuestionLinMotDataset.register_question_generator(
        "movement_in_reverse", MovementInReverseQGen)
    
    MultiQuestionLinMotDataset.register_question_generator(
        "total_obj_count", HowManyObjsQGen)
    MultiQuestionLinMotDataset.register_question_generator(
        "mov_obj_count", HowManyMovingObjsQGen)
    MultiQuestionLinMotDataset.register_question_generator(
        "still_obj_count", HowManyStillObjsQGen)
    MultiQuestionLinMotDataset.register_question_generator(
        "how_many_of_col", HowManyObjsPresentColorQGen)
    MultiQuestionLinMotDataset.register_question_generator(
        "how_many_of_col_absent", HowManyObjsAbsentColorQGen)
    MultiQuestionLinMotDataset.register_question_generator(
        "how_many_of_shape", HowManyObjsPresentShapeQGen)
    MultiQuestionLinMotDataset.register_question_generator(
        "how_many_of_shape_absent", HowManyObjsAbsentShapeQGen)
    
    ImgSplicingDataset.register_question_generator(
        "scenes_descr", ScenesDescrQGen)
    ImgSplicingDataset.register_question_generator(
        "describe_nth_scene", DescribeNthSceneQGen)
    ImgSplicingDataset.register_question_generator(
        "count_scenes", CountScenesQGen)
    ImgSplicingDataset.register_question_generator(
        "descr_adjacent_scene", DescrAdjacentSceneQGen)
    ImgSplicingDataset.register_question_generator(
        "fr_level_annotate", FrLvlAnnotationQGen)
    ImgSplicingDataset.register_question_generator(
        "before_or_after", BeforeOrAfterQGen)
    ImgSplicingDataset.register_question_generator(
        "which_first_last", WhichHappensFirstLastQGen)
    ImgSplicingDataset.register_question_generator(
        "which_is_or_isnt", WhichIsOrIsntInVideoQGen)
        
    ImgSplicingDatasetWithObjInfo.register_question_generator(
        "count_instances", HowManyInstOfClassQGen)

    dataset_kwargs = json.loads(dataset_kwargs_as_dict_str)
    if(dataset_type == DatasetType.ORIGINAL_LONGVU):
        train_dataset = LazySupervisedDataset(
            tokenizer=tokenizer, data_args=data_args, **dataset_kwargs)
    elif(dataset_type == DatasetType.LINEAR_MOTION):
        wrapped = motion_dataset_from_yaml(**dataset_kwargs)
        train_dataset = WrappedMotionDatasetLazy(wrapped,
            data_args=data_args, tokenizer=tokenizer)
    elif(dataset_type == DatasetType.SCENE_DESCR):
        wrapped = scene_descr_dataset_from_yaml(**dataset_kwargs)
        train_dataset = WrappedSceneDescrDataset(wrapped, data_args=data_args,
                                                 tokenizer=tokenizer)
    elif(dataset_type == DatasetType.COCO_IMG_SPLICING):
        wrapped = spliced_coco_imgs_dataset_from_yaml(**dataset_kwargs)
        train_dataset = WrappedSplicedImgsDatasetWithMasks(
            wrapped, data_args=data_args, tokenizer=tokenizer)
    else:
    ...
    data_loader = None
    if(dataset_type == DatasetType.LINEAR_MOTION):
        sample_probs = (
            train_dataset.wrapped_dataset.get_sampling_prob_per_sample())        
        if(sample_probs is not None):
            sampler = WeightedRandomSampler(sample_probs, len(train_dataset))
    ...
    ret = {"train_dataset": train_dataset, "eval_dataset": None,
           "data_collator": data_collator, 
           **({"train_dataloader": data_loader} if 
           data_loader is not None else {})}
    return ret


TRAINING_COMPLETE_FLAG_FILE = "training_complete.flag"
def train() -> None:
    ...
    transformers.set_seed(training_args.seed + global_rank)
    ...
    data_module = make_supervised_data_module(
        dataset_type=data_args.dataset_type, tokenizer=tokenizer, 
        data_args=data_args, 
        dataset_kwargs_as_dict_str=data_args.dataset_kwargs_as_dict_str,
    ...
    if(ckpts):
        valid_count = 0
        for potential_ckpt in ckpts:
            full_path = os.path.join(training_args.output_dir, potential_ckpt)
            flag_file = os.path.join(full_path, VALID_CKPT_FILE)
            if(not os.path.exists(flag_file)): #this checkpoint was interrupted
    ...
    if(((not is_available() or not is_initialized())
        or (get_rank() == 0)) and training_args.slurm_dir != ""):
        if(not os.path.exists(training_args.slurm_dir)):
            os.mkdir(training_args.slurm_dir)
        with(open(
            os.path.join(training_args.slurm_dir, 
                         TRAINING_COMPLETE_FLAG_FILE), "w") as f):
