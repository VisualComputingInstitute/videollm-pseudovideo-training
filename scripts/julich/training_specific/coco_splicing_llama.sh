...
copied_batch_file=${copy_code_to}/scripts/julich/sbatch_train.sh
...
"./pretr_checkpoints/cambrian/LongVU_Llama3_2_3B_img" \
...
"--repeat_first_frame_only" "True" \
"--dataset_kwargs_as_dict_str" \
"{${s}\"coco_yaml_path${s}\": \
${s}\"./data_configs/splicing/coco_captioning_julich.yaml${s}\", \
${s}\"question_yaml_path${s}\": \
${s}\"./data_configs/splicing/q1_complexity_ablations/q_complexity1_500k_samples.yaml${s}\"}")
...
if [ "$am_i_testing" -eq 1 ]; then
    ...
    runtime="00:30:00"
    num_nodes=2
    data_args+=("--debug_print_question" "True")
    ...
    runtime="04:00:00"
    num_nodes=8
    data_args+=("--debug_print_question" "False")
fi
