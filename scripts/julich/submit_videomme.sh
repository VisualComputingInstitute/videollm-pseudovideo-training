...
log_file=${slurm_dir}/slurm_eval_videomme.txt
dataset_loc=${PROJECT}/workspaces/datasets/VideoMMe
sbatch --job-name="${experiment_name}_eval_videomme" \
    --output=$log_file --time="01:00:00" \
    scripts/julich/sbatch_videomme.sh $experiment_dir \
    $dataset_loc "$@"